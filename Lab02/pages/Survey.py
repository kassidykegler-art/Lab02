...
st.title("Class Subject and Workload Survey 📝")
...
with st.form("survey_form"):
    ...
    category_input = st.text_input("Enter a Subject:")
    value_input = st.text_input("Enter the number of hours you spend on work outside of that class:")
...
if st.button("🧹 Clear CSV Data"):
    if os.path.exists("data.csv"):
        pd.DataFrame(columns=["Category", "Value"]).to_csv("data.csv", index=False)
        st.success("✅ 'data.csv' has been cleared successfully!")
    else:
        st.warning("No 'data.csv' file found to clear.")
